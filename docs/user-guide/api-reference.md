...
equivalent knob in the `jpeg-encoder` encoder this service routes JPEG
through; it does not implement trellis quantisation at all.
...
dav1d for decode; vendored C, built via `cmake`). On the encode side this
...
**TODO(re-measure)**. `image`'s own `avif`/`avif-native` features
are still not used for either direction (`ImageFormat`'s doc comment,
