...
//! binary (and `decoder-noasm`, its twin) as a subprocess per round.
//!
//! This file is intentionally near-identical to `decoder-noasm/src/main.rs`.
...
fn run() -> Result<(), String> {
    ...
    let path = args
        .next()
        .ok_or("usage: decoder-asm <avif-path> <iterations>")?;
